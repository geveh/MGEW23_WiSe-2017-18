...
(1-p)^n
```
This is the probability for no 1,000-year tornado to occur.

Result: The probability for at least one tornado to occur is 63%. 
**Statement verified!**
...
(obs <- sum(y >= 0.999))
```
In this simulation one 1,000-year tornado could be observed (result=1). If we would generate new numbers and count them, another number of hurricanes could be our result.
