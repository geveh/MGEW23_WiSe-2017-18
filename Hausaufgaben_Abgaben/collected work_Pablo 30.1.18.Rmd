...
likelihood = function(n,k,Theta){return((factorial(n)/factorial(k)*factorial(n-k))*Theta^k*(1-Theta)^(n-k))} #Theta <- Probability; k <- observed value; <-n fixed value
like <- likelihood(20,3,Theta)
plot(Theta,like,type = "h")

DataEvidence = (like*prior)/(1-like)*(1-prior)
#Posterior plot
Posterior1=like*prior/DataEvidence
